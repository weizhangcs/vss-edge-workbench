...
class CreativeBatch(TimeStampedModel):
    ...
    inference_project = models.ForeignKey(InferenceProject, on_delete=models.PROTECT, verbose_name="源推理项目")
...
class CreativeProject(BaseProject):
    ...
    status = FSMField(max_length=30, choices=STATUS, default=STATUS.PENDING, verbose_name="创作项目状态")
    ...
    batch = models.ForeignKey(
        CreativeBatch, on_delete=models.SET_NULL, null=True, blank=True, related_name="projects", verbose_name="所属批次"
    )
    ...
    #   "edit": { ... }
    # }
    auto_config = models.JSONField(null=True, blank=True, verbose_name="自动化全流程配置 (Auto-Pilot)")

    # --- 步骤 1 产出物 ---
    narration_script_file = models.FileField(
        upload_to=get_creative_output_upload_path, blank=True, null=True, verbose_name="解说词脚本 (JSON)"
    )

    # --- [V1.2.1 新增] 本地化产出物 ---
    localized_script_file = models.FileField(
        upload_to=get_creative_output_upload_path, blank=True, null=True, verbose_name="本地化/翻译脚本 (JSON)"
    )
    ...
    edit_script_file = models.FileField(
        upload_to=get_creative_output_upload_path, blank=True, null=True, verbose_name="剪辑脚本 (JSON/EDL)"
    )

    # --- [新增] 步骤 4 产出物 ---
    final_video_file = models.FileField(
        upload_to=get_creative_output_upload_path, blank=True, null=True, verbose_name="最终合成视频 (MP4)"
    )
