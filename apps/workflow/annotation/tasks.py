...
def create_label_studio_project_task(project_id: str):
    ...
    if success:
        ...
        logger.info(f"成功为 AnnotationProject (ID: {project_id}) 创建并关联了 Label Studio 项目 (LS ID: {ls_project_id})。")
...
def export_l2_output_task(project_id: str):
    ...
    try:
        ...
        jobs_to_complete = AnnotationJob.objects.filter(
            project=project, job_type=AnnotationJob.TYPE.L2L3_SEMANTIC, status=BaseJob.STATUS.PROCESSING
        )
        completed_count = 0
...
def generate_narrative_blueprint_task(project_id: str):
    ...
    try:
        ...
        l1_jobs = AnnotationJob.objects.filter(
            project=project, job_type=AnnotationJob.TYPE.L1_SUBEDITING
        ).select_related("media")

        for l1_job in l1_jobs:
            l2_job = AnnotationJob.objects.filter(
                project=project, media=l1_job.media, job_type=AnnotationJob.TYPE.L2L3_SEMANTIC
            ).first()
        ...
        project.final_blueprint_file.save(
            blueprint_filename, ContentFile(blueprint_content.encode("utf-8")), save=False
        )
...
def calculate_local_metrics_task(project_id: str):
    ...
    try:
        ...
        # 6. 保存到新字段 (延迟 save)
        project.local_metrics_result_file.save(file_name, ContentFile(report_json_str.encode("utf-8")), save=False)
